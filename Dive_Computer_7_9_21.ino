...
void loop() {
  ...
  if ((newTime - oldTime5) > timeout5)  // Pressure Sensor
  {
    ...
    if (abs(feet_SW) >= 100.0)
    {
      oled.setCursor(0, 1);
      oled.print(abs(feet_SW), 1);
    }
    else if (10.0 <= abs(feet_SW) && abs(feet_SW) < 100.0)
    {
      oled.setCursor(0, 1);
      oled.print(" ");
      oled.print(abs(feet_SW), 1);
    ...
    if ((abs(feet_SW) > 2))   ////////////// Start of Dive ////////////////
    {
      ...
      long currentDiveTimeHMS = ((hourFromUnix * 60) + minuteFromUnix);
      totalTimeDown = currentDiveTimeHMS + resNTime;
      Serial.print(totalTimeDown); Serial.print('\t'); Serial.println(resNTime);
      goToSurface |= getGoToSurface(maxDepth, totalTimeDown);
      if (goToSurface == 1)
      {
        oled.setCursor(0, 3);
...
char readGroup(float maxDepth, long* diveDuration)
{
  ...
  while (strtol(ptr, &str, 10) < diveDuration)
  {
    ...
    letterGroup++;
    if (letterGroup > 'Z') letterGroup = 'Z';
  }
  return letterGroup;
}
...
{
  long D = 0;
  if (maxDepth > 140) D = 140;
  else if (maxDepth > 130) D = 140;
  else if (maxDepth > 120) D = 130;
  else if (maxDepth > 110) D = 120;
  else if (maxDepth > 100) D = 110;
  else if (maxDepth > 90) D = 100;
...
bool getGoToSurface(long maxDepth, long currentDiveTime)
{
  long depth = roundNum(maxDepth);
  
  if (depth == 35)
  {
    if(currentDiveTime >= 205) return true;
  }
  else if (depth == 40)
  {
    if(currentDiveTime >= 140) return true;
  }
  else if (depth == 50)
  {
    if(currentDiveTime >= 80) return true;
  }
  else if (depth == 60)
  {
    if(currentDiveTime >= 55) return true;
  }
  else if (depth == 70)
  {
    if(currentDiveTime >= 40) return true;
  }
  else if (depth == 80)
  {
    if (currentDiveTime >= 30) return true;
  }
  else if (depth == 90)
  {
    if (currentDiveTime >= 25) return true;
  }
  else if (depth == 100)
  {
    if (currentDiveTime >= 20) return true;
  }
  else if (depth == 110)
  {
    if(currentDiveTime >= 16) return true;
  }
  else if (depth == 120)
  {
    if (currentDiveTime >= 13) return true;
  }
  else if (depth == 130)
  {
    if (currentDiveTime >= 10) return true;
  }
  else // means 131 or higher
  {
    if (currentDiveTime >= 8) return true;
  }
  return false;
}
